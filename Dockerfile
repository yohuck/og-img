...
WORKDIR /src
COPY ["og-image-generator.csproj", "."]
RUN dotnet restore "./og-image-generator.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "og-image-generator.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "og-image-generator.csproj" -c Release -o /app/publish /p:UseAppHost=false
